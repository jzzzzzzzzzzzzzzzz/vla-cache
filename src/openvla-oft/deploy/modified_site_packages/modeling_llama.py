...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # keep softmax in native dtype (bfloat16) to halve peak memory (~43 MB vs ~86 MB)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # ── Prune Class B tokens (before any decoder layer) ─────────────────────
        # Class B = low pixel-similarity AND low text-attention.  These tokens are
        # irrelevant to the model's decision; remove them from the query sequence
        # and block attention to their stale KV entries in the prompt cache.
        pruned_seq_len = inputs_embeds.shape[1]   # N; shrinks to N-|B| if prune active
        prune_patches = getattr(self.config, 'prune_patches', None)
        if prune_patches is not None and inputs_embeds.size(1) != 1:
            _dev = hidden_states.device
            _prune_dev = prune_patches.to(_dev)
            _keep = ~torch.isin(cache_position, _prune_dev)
            # 1. Physically shorten Q sequence (all subsequent layers process N-|B| tokens)
            hidden_states  = hidden_states[:, _keep, :]
            cache_position = cache_position[_keep]
            position_ids   = cache_position.unsqueeze(0)
            pruned_seq_len = hidden_states.shape[1]
            # 2. Mask B columns so no remaining Q can attend to B's stale KV in cache
            _N_orig = inputs_embeds.shape[1]   # original sequence length (before B removal)
            _min_dt = torch.finfo(hidden_states.dtype).min
            if causal_mask is not None:
                causal_mask = causal_mask[:, :, _keep, :]          # drop B rows
                _b_cols = torch.isin(
                    torch.arange(causal_mask.shape[-1], device=_dev), _prune_dev
                )
                causal_mask = causal_mask.clone()
                causal_mask[:, :, :, _b_cols] = _min_dt
            else:
                # SDPA returned None (key_value_length==query_length shortcut).
                # Leave causal_mask=None — VLA-Cache uses full attention (no mask) and
                # the model relies on that pattern.  B's stale KV in the cache
                # participates in attention like A-class stale KV does in VLA-Cache.
                pass
        # ── End Prune B ──────────────────────────────────────────────────────────
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.config.proportion_attn_var is not None and inputs_embeds.size(1) != 1 and layer_idx in self.pruning_loc:
                ...
                if last_reusable_patches.size(0) <= selected_reusable_patches.size(0):
                    ...
                    assert new_cache_position.size(0) + selected_reusable_patches.size(0) == pruned_seq_len, f"{new_cache_position.size(0)} + {selected_reusable_patches.size(0)} != {pruned_seq_len}"
